...
This is not equal to $\Delta_!\Mm$ in general(\tk is this true?), but in the case where $\Mm$ is a $\Dd$-module, it can be related: Let $\widehat{\Delta}$ be the formal completion of $X^2$ around $\Delta$.  Then, if $\Mm$ is a $\Dd$-module, there is an isomorphism
\begin{align}
...
A similar construction can be made to identify $\Delta_!\Mm \rightarrow T_*\Delta_!\Mm$.
...
\item An ``identity map'' $\Omega_X \rightarrow \Aa$.
\end{itemize}
