...
\label{sec:mot}
%nocites: Kac's for beginners book; FLM VOAs&Monster; BD Chiral algs; chiral Koszul 
...
A particularly important case of vertex operator algebra is the \textit{rational vertex operator algebra}, where the category of representations is semisimple; that is, every representation decomposes into a direct sum of simple representations.  In this case\footnote{assuming some technical conditions}, the conformal blocks on the torus are generated by the graded characters of the representations, and as such the vector space generated by the latter has an action of $SL_2(\ZZ)$.
...
(note that it is sometimes also denoted as $\delta(z-w)$, ) and the fact that it satisfies
\[(z-w)\delta(z,w)=0. \]
...
The functions $\phi_{J/I}$ should be functorial, and the unit works as follows: for any $f \in \Ff_1(U)$, the value \[1 \boxtimes f \in \Ff_1(U) \boxtimes \Ff_1(U) \subseteq \Ff_1 \boxtimes \Ff_1(U^2 \setminus \Delta) \simeq \Ff_2(U^2 \setminus \Delta)\] will correspond to a value
...
To prove this module structure is indeed a module structure, we can note an involution in the Lie algebra $\Gamma(\CC^\times,h(\Vv \otimes \Omega))$: this is the vector space spanned by all elements $A_{(n)}$.  The involution $\tau$ is defined using the map $t \mapsto t^{-1}$ using the above relation between nearby local coordinates; for $A \in V$, if $\deg A=\Delta$, then
\[\tau(A_{(n)})=(-1)^{\Delta+1} \sum_{m \ge 0} \frac{1}{m!}(L_1^mA)_{(2\Delta-n-m-2)}. \]
(This calculation also implies the global sections on $\PP^1 \setminus 0$ from the more familiar $\PP^1 \setminus \infty$) The adjoint of this involution's action on $V$ provides an action of $\Gamma(\CC^\times,h(\Vv \otimes \Omega))$ on $V^\vee$; by showing the involution also preserves normal-ordered products, a theorem related to the reconstruction theorem will show that this necessarily extends to a module structure on $V^\vee$.
...
%mildly upset that I can't use Unicode here even with XeTeX
A formal version of \v{C}ech cohomology leads to these identities:
\begin{align}
